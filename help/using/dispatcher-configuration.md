...
## Abilitare sessioni sicure - `/sessionmanagement` {#enabling-secure-sessions-sessionmanagement}

>[!CAUTION]
>
>`/allowAuthorized` Imposta su `"0"` nella sezione `/cache` per abilitare questa funzione. Come descritto nella sezione [“Caching quando viene utilizzata l’autenticazione”](#caching-when-authentication-is-used), quando imposti `/allowAuthorized 0 ` le richieste che includono informazioni di autenticazione **non** vengono memorizzate nella cache. Se viene richiesta la memorizzazione in cache dipendente dalle autorizzazioni, consulta la pagina [Memorizzazione in cache di contenuto protetto](https://experienceleague.adobe.com/it/docs/experience-manager-dispatcher/using/configuring/permissions-cache).
...
>[!CAUTION]
>
> Durante la configurazione del sottoparametro della directory, **non** puntare alla cartella principale (`/directory "/"`), in quanto ciò può causare gravi problemi. È sempre necessario specificare il percorso della cartella in cui sono memorizzate le informazioni della sessione. Ad esempio:
...
## Definire i motori di rendering di pagina {#defining-page-renderers-renders}
...
#### La parte di riga di richiesta delle richieste HTTP {#the-request-line-part-of-http-requests}
...
#### Virgolette doppie e apici {#double-quotes-vs-single-quotes}
...
#### Esempio di filtro: utilizza espressioni regolari {#example-filter-using-regular-expressions}

Questo filtro abilita le estensioni nelle directory di contenuto non pubblico utilizzando un’espressione regolare, qui definita tra apici singoli:

```xml
/005  {  /type "allow" /extension '(css|gif|ico|js|png|swf|jpe?g)' }
```

#### Esempio di filtro: filtra elementi aggiuntivi dell’URL di una richiesta {#example-filter-filter-additional-elements-of-a-request-url}
...
### Limitare stringhe di query {#restricting-query-strings}
...
### Testare la sicurezza di Dispatcher {#testing-dispatcher-security}
...
Aggiungendo `/loadOnStartup 0` (consulta l’esempio seguente) è possibile disabilitare il caricamento degli URL personalizzati all’avvio.
...
## Inoltrare richieste di distribuzione del contenuto: `/propagateSyndPost` {#forwarding-syndication-requests-propagatesyndpost}
...
### Denominare lo statfile {#naming-the-statfile}
...
### Distribuzione di documenti obsoleti quando si verificano errori {#serving-stale-documents-when-errors-occur}
...
### Ignorare parametri URL {#ignoring-url-parameters}
...
>[!NOTE]
>
>Durante la configurazione della proprietà glob, questa deve corrispondere al nome del parametro della query. Ad esempio, se desideri ignorare il parametro &quot;p1&quot; dal seguente URL `http://example.com/path/test.html?p1=test&p2=v2`, la proprietà glob deve essere:
...
### Limitazione del numero di interventi sul file .stat {#throttling-stat-file-touching}
...
Configurare il comportamento di Dispatcher quando il server di rendering restituisce l’errore 500 o non è disponibile.

### Specificare una pagina di verifica dello stato di integrità {#specifying-a-health-check-page}
...
### Specificare il ritardo nei nuovi tentativi di richiesta della pagina {#specifying-the-page-retry-delay}

La proprietà `/retryDelay` imposta il tempo (in secondi) che Dispatcher deve attendere prima di ogni turno di tentativi di connessione successivo con i rendering nella farm. Per ogni turno, il numero massimo di tentativi di Dispatcher di connettersi a un motore di rendering è pari al numero di rendering nella farm.
...
Usa il parametro `/ignoreEINTR`, se la tua istanza dispone di questa configurazione e il registro contiene il seguente messaggio:
...
## Confermare il funzionamento di base {#confirming-basic-operation}
...
Il metodo HTTP non è un metodo GET o HEAD. Dispatcher presuppone che l’output contenga dati dinamici che non devono essere memorizzati nella cache.
