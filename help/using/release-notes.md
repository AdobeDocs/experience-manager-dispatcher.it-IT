...
description: Note sulla versione specifiche per Adobe Experience Manager Dispatcher.
...
Consulta [Piattaforme supportate](https://experienceleague.adobe.com/en/docs/experience-manager-64/deploying/introduction/technical-requirements) per ulteriori informazioni su requisiti e prerequisiti.

L’Adobe consiglia di utilizzare la versione più recente di Dispatcher AEM per sfruttare le funzionalità e le correzioni di bug più recenti e ottenere le migliori prestazioni possibili.
...
* DISP-1014 - elaborazione response.location che porta a un reindirizzamento non valido.
...
* DISP-937 registra tutti i selettori.
* DISP-998 rende configurabile il caricamento di URL personalizzati all’avvio.
...
* DISP-826 - Supporta il recupero di URI con una stringa di query
...
* DISP-746 - Le stringhe non etichettate nel dispatcher.any devono generare un avviso
...
* DISP-723 - Timeout implicito di 10 minuti (e nuovo tentativo) quando i timeout di rendering sono impostati su 0
...
* DISP-693 - Aggiornamento dei dettagli di architettura dei server Solaris™ nella pagina di download di Dispatcher
...
* DISP-705 - Arresto anomalo di Dispatcher a causa di una vulnerabilità di tipo Double Free o Danneggiamento (`fasttop`)
